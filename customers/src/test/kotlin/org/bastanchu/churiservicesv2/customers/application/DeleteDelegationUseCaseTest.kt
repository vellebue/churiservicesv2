...
import org.bastanchu.churiservicesv2.customers.application.service.DeleteDelegationService
...
class DeleteDelegationUseCaseTest {

    @Mock
    private lateinit var delegationRepository: CustomerDelegationRepository

    @InjectMocks
    private lateinit var service: DeleteDelegationService

    @Test
    fun `should delete an existing delegation`() {
        val existing = CustomerDelegation(
            delegationId = 50L,
            customerId = 1L,
            orderId = 0,
            name = "Madrid Branch",
        ...
        verify(delegationRepository).deleteById(50L)
    }
