...
import org.bastanchu.churiservicesv2.customers.application.service.PingService
...
class PingUseCaseTest {
    ...
    fun `should return root status only when fullMode is false`() {
        ...
        assertEquals("churiservices-customers", result.componentName)
    ...
    fun `should include running database dependency when fullMode is true`() {
        `when`(databaseHealthChecker.check()).thenReturn(
            DatabaseHealthInfo(
                running = true,
                componentName = "churiservices-customers-db",
                version = "16.2"
            )
        )
        ...
        assertEquals("churiservices-customers-db", dbStatus.componentName)
